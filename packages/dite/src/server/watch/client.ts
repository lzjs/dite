...
class TscWatchClient extends EventEmitter {
  start(...args: any[]) {
    this.tsc = child_process.fork(require.resolve('./tsc-watch.js'), args, {
      stdio: 'inherit',
    });
    this.tsc.on('message', (msg: any) =>
      this.emit(...deserializeTscMessage(msg)),
    );
    this.tsc.on('exit', (code: any, signal: any) =>
...
function deserializeTscMessage(strMsg: string) {
  ...
  return [
    strMsg.substring(0, indexOfSeparator),
    strMsg.substring(indexOfSeparator + 1),
  ];
}
